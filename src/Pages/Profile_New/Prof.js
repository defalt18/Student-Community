...
import { ProfilePhotos } from '../../Components'
...
function Prof(props) {
	...
	useEffect(() => {
		...
		db.collection('posts')
			.where('UID', '==', uid)
			.get()
			.then((resp) => snp(resp.kf.docChanges.length))
		...
		<>
			...
			>
				...
				>
					...
					>
						...
						>
							{user.uid === props.match.params.id ? (
								...
								<d onClick={followme}>Following</d>
							) : (
								<d onClick={followme}>Follow</d>
							)}
						</Button>
					</div>
					<h1
					...
					>
						{userabt.firstName} {userabt.lastName}
					</h1>
					<div
						style={{
							display: 'flex',
					...
					>
						...
						>
							...
							<h2>
								{initialValues.city}, {initialValues.state}
							</h2>
						</div>
						<div
							style={{
						...
						>
							...
							<d style={{ fontSize: '20px', textTransform: 'uppercase' }}>
								{initialValues.degree} {initialValues.batch},{' '}
						...
						{role === 1 && (
							<TabPanel value={value} index={3}>
								<h2 style={{ margin: '0 5px', marginBottom: '10px' }}>
									Events by {initialValues.studentid} will show here
								</h2>
								<div
									style={{
										display: 'flex',
