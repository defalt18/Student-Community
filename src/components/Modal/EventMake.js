...
function EventMake({ handleClose, imgus }) {
	...
	const handleSubmit = () => {
		db.collection('events')
			.doc((det.name + user.uid).toString())
			.set({
				name: det.name,
				poster: det.poster,
				team: det.team,
		...
		>
			...
			<div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
				<div style={{ paddingLeft: '3%', width: '70%' }}>
					...
					<p>
						Events are what holds the community together where students come
						together to showcase their talents. It is the responsibility of the
						clubs to make sure these events are well organised and encourage
						significant participations.
						<br />
						<br />
						Clubs can see participants info under Profile {'=>'} Events {'=>'}{' '}
						See Participants
					</p>
				</div>
			...
			>
				...
				<FileDrop onTargetClick={onTargetClick}>
					<p style={{ margin: '0', padding: '5vh 0' }}>
						{Img ? Img.name : 'Drop your event poster'}
					</p>
				</FileDrop>
				<TextField
					id='venue'
					onChange={handleCh}
					value={det.google_form}
					name='google_form'
				...
				/>
				<p style={{ margin: 0 }}>
					*In case you want more information from the participants, we allow the
					google form links as well!
				</p>
				<Button
